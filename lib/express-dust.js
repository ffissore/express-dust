...
res.render = function(view, opts, fn) {
    ...
    var self = this,
        ...
        asyncHelpers = app.asyncViewHelpers || {},
    ...
    function done() {
        context = context.push(self.__asyncHelpers || {});
    ...
    if (false !== context.get('asyncHelpers') && !self.__asyncHelpers) {
        self.__asyncHelpers = {};
        function runAsyncHelpers(keys) {
            if (!keys.length) done();
            else {
                var key = keys.shift();
                asyncHelpers[key].call(
                    self.app,
                    self.req,
                    self,
                    function(err, result) {
                        if (err) self.req.next(err);
                            ...
                            self.__asyncHelpers[key] = result;
                            runAsyncHelpers(keys);
                        }
                    }
                );
            }
        }
        runAsyncHelpers(Object.keys(asyncHelpers));
    } else done();
};
